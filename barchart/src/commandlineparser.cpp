    ...
    int argc, char** argv, const std::vector<std::string>& allowedArguments)
{
    ...
    for (int i = 0; i < argc; i++)
    {
        ...
        if (arg[0] == '-')
        {
            // Check there is a next argument to use as the value for this
            // option
            if (i == (argc - 1))
                throw std::runtime_error("Couldn't parse arguments, argument "
                                         "specified with no value");

            // Check this argument is supported
            if (std::find(allowedArguments.begin(), allowedArguments.end(),
                          arg) == allowedArguments.end())
...
const std::string& Arguments::get(const std::string& option,
